...
from ..isolators import IdleIsolator, CycleLimitIsolator, CPUFreqThrottleIsolator, CacheIsolator, \
    SchedIsolator, AffinityIsolator
from ...workload import Workload
from ...utils.machine_type import NodeType


class XeonPolicy(IsolationPolicy):
    ...
    def choose_next_isolator(self) -> bool:
        ...
        if self._dom_res_cont is ResourceType.CPU:
            isolator = self._isolator_map[AffinityIsolator]         # CPU Allocation (to LC wl)
        elif self._dom_res_cont is ResourceType.CACHE:
            isolator = self._isolator_map[CacheIsolator]            # HW Cache Partitioning
